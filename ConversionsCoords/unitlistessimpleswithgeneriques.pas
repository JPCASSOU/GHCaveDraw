...
(*
function TListeSimple<T>.PopElement: T;
var
  Nb: integer;
begin
  ...
  if (Nb > 0) then
  begin
    Result := self.GetElement(Nb-1);
    self.RemoveLastElement();
  end;
end;

procedure TListeSimple<T>.PushElement(const E: T);
begin
  self.AddElement(E);
end;
procedure TListeSimple<T>.PushElements(const AE: array of T);
var i, n: integer;
begin
  n := 1 + High(AE);
  for i := 0 to n - 1 do self.AddElement(AE[i]);
end;
//*)
//******************************************************************************
{ TTableEntrees }
